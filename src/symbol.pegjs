kicad_symbol_lib
  = '(' _
    type:'kicad_symbol_lib' _
    value:( val:(
      version /
      generator /
      generator_version /
      kicad_symbol ) _ { return val })*
    ')' _ {
    return {type, value}
  };
...
generator
  = '(' _ type:'generator' _ value:string _ ')' {
    return {type, value };
  }
...
pin_numbers
  = '(' _ type:'pin_numbers' _ rest:(val:(offset / hide_token) _ { return val })* ')' {
    return {type, value: rest}
  }

pin_names
  = '(' _ type:'pin_names' _ rest:(val:(offset / hide_token) _ { return val })* ')' {
    return {type, value: rest}
  }

hide_token = value:('hide') _ {
    return { type: "hide", value: {type: "boolean", value: value === "hide" }}
}

offset
 = '(' _ type:'offset' _ value:number _ ')' {
    return {type, value}
 }
...
property
  = '(' _ 'property' _ name:string _ value:string _ at:at _ (effects:effects _)? ')' {
    var values = [
      {type: "key", value: name },
      {type: "value", value},
      at
...
graphic_item
  = arc / circle / bezier / polyline / rectangle / text

arc
  = '(' _ 'arc' _ start:start _ mid:mid _ end:end _ stroke_definition:stroke_definition _ fill:fill _ ')'{
...
circle
  = _ '(' _ 'circle' _ center:center _ radius:radius _ stroke_definition:stroke_definition _ fill:fill _ ')' _ {
...
polyline
  = _ '(' _ 'polyline' _ pts:pts _ stroke_definition:stroke_definition _ fill:fill _ ')' _ {
...
rectangle
  = '(' _ 'rectangle' _ start:start _ end:end _ stroke_definition:stroke_definition _ fill:fill _ ')' {
...
pin
  = _ '(' _ 'pin' _ rest:(val: (
    ...
    hide_token /
    pin_name /
    pin_number) _ { return val })+
  ')' _ {
    return {type:"pin", value:rest}
  }

pin_electrical_type
  = value:('input' / 'output' / 'bidirectional' / 'tri_state' / 'passive' / 'free' / 'unspecified' / 'power_in' / 'power_out' / 'open_collector' / 'open_emitter' / 'no_connect') {
    return {type:"pin_electrical_type", value: { type: "string", value }}
  }

pin_graphic_style
  = value:('inverted_clock' /'line' / 'inverted' / 'input_low' / 'clock_low' / 'clock' / 'output_low' / 'edge_clock_high' / 'non_logic') {
...
alternate
  = '(' _ type:"alternate" _ name:symbol _ rest:(val: (
    pin_graphic_style /
    pin_electrical_type ) _ { return val })+
  ')' _ {
    return {type, value:[
...
length
  = '(' _ type:'length' _ value:number _ ')' {
    return {type, value}
  }

unit_name
  = _ '(' _ 'unit_name' _ string _ ')' _
...
stroke_definition
  = _ '(' _ type:'stroke' _ rest:(val:(width / stroke_type/ color) _ { return val } )* ')' {
    return {type, value: rest}
  }
...
stroke_type
  = '(' _ type:'type' _ value:('dash' / 'dash_dot' / 'dash_dot_dot' / 'dot' / 'default' / 'solid') _ ')' {
